...
# FIX: importar rutas centralizadas — eliminar las dos definiciones duplicadas
# que había antes (una de modulos.rutas y otra con _get_db_path local)
from modulos.rutas import get_db_path, get_config_path
DB_PATH     = get_db_path()
...
def abrir_ventana_suscripciones(parent):
    ...
    ctk.CTkLabel(frame_header, text="Gestion de Suscripciones",
                 font=("Segoe UI", 28, "bold")).pack(side="left", padx=20, pady=20)
    ctk.CTkLabel(frame_header, text="Control de pagos, vencimientos e ingresos",
                 font=("Segoe UI", 14), text_color="gray70").pack(side="left")
    ctk.CTkButton(frame_header, text="<- Volver al menu", width=170, height=40,
    ...
    ctk.CTkLabel(frame_grafico, text="Ingresos mensuales",
    ...
    def abrir_popup_agregar():
        ...
        campos = ["ID Cliente", "ID Membresia", "Precio total", "Monto pagado",
        ...
        def guardar():
            ...
            if not val_cliente or not val_membresia or not val_precio or not val_pagado:
                lbl_error.configure(text="Completa todos los campos obligatorios."); return
            try:
                cliente   = int(val_cliente); membresia = int(val_membresia)
                precio    = float(val_precio); pagado    = float(val_pagado)
            except ValueError:
                lbl_error.configure(text="ID debe ser entero, precio y pagado deben ser numeros."); return
            fecha_validada = validar_fecha(val_fecha)
            if fecha_validada is False:
                lbl_error.configure(text="Formato de fecha inválido. Usa YYYY-MM-DD, DD/MM/YYYY o DD-MM-YYYY."); return
            asignar_membresia(cliente, membresia, precio, pagado, fecha_validada)
            recargar_tabla(); cargar_grafico_ingresos(); popup.destroy(); traer_al_frente()
            messagebox.showinfo("Exito",
                "Suscripcion agregada correctamente.\nPresiona 'Actualizar' en el dashboard para ver el contador.",
                parent=ventana)

        ctk.CTkButton(popup, text="Guardar suscripcion", height=42,
                      font=("Segoe UI", 13, "bold"), fg_color="#1a7a1a",
    ...
    def abrir_popup_editar_fechas():
        ...
        popup = ctk.CTkToplevel(ventana)
        popup.title("Editar fechas"); popup.geometry("460x260")
        popup.resizable(False, False)
        popup.attributes("-topmost", True)
        popup.after(300, lambda: popup.attributes("-topmost", False))
        popup.lift(); popup.focus_force()
        ...
        def guardar_fechas():
            ...
            if fi is False or fv is False:
                lbl_err.configure(text="Formato invalido. Usa YYYY-MM-DD, DD/MM/YYYY o DD-MM-YYYY."); return
            if not fi or not fv:
                lbl_err.configure(text="Ambas fechas son obligatorias."); return
            con = sqlite3.connect(DB_PATH)
            con.execute("UPDATE suscripciones SET fecha_inicio=?, fecha_vencimiento=? WHERE id=?",
                        (fi, fv, int(id_sus)))
            con.commit(); con.close()
            popup.destroy(); recargar_tabla(); cargar_grafico_ingresos(); traer_al_frente()
            messagebox.showinfo("Guardado", "Fechas actualizadas correctamente.", parent=ventana)

        ctk.CTkButton(popup, text="Guardar fechas", height=40,
                      font=("Segoe UI", 13, "bold"), fg_color="#0d6efd",
                      hover_color="#0b5ed7", command=guardar_fechas).pack(pady=12)
    ...
    for i, (txt, cmd) in enumerate(botones_vista):
        ctk.CTkButton(frame_botones, text=txt, width=165, height=40,
                      command=cmd).grid(row=0, column=i, padx=6, pady=(10, 4))

    ctk.CTkButton(frame_botones, text="+ Agregar suscripcion", width=185, height=40,
                  fg_color="#1a7a1a", hover_color="#145214",
                  command=abrir_popup_agregar).grid(row=1, column=0, padx=6, pady=(4, 10))
    ctk.CTkButton(frame_botones, text="Eliminar suscripcion", width=185, height=40,
                  fg_color="#C0392B", hover_color="#922B21",
                  command=eliminar_seleccionada).grid(row=1, column=1, padx=6, pady=(4, 10))
    ctk.CTkButton(frame_botones, text="Editar fechas", width=185, height=40,
                  fg_color="#0d6efd", hover_color="#0b5ed7",
                  command=abrir_popup_editar_fechas).grid(row=1, column=2, padx=6, pady=(4, 10))

    # ---------- FORMULARIO FIJO ----------
    frame_asignar = ctk.CTkFrame(scroll, corner_radius=18)
    frame_asignar.pack(fill="x", padx=20, pady=20)

    ctk.CTkLabel(frame_asignar, text="Asignar nueva membresia",
                 font=("Segoe UI", 20, "bold")).grid(row=0, column=0, columnspan=2, pady=(15, 10))
    ...
    def asignar():
        ...
        val_p  = entry_precio.get().strip();  val_pa = entry_pagado.get().strip()
        if not val_c or not val_m or not val_p or not val_pa:
            ...
            traer_al_frente(); return
        try:
            cliente   = int(val_c); membresia = int(val_m)
            precio    = float(val_p); pagado   = float(val_pa)
        except ValueError:
            messagebox.showerror("Error", "ID debe ser entero, precio y pagado deben ser numeros.", parent=ventana)
            traer_al_frente(); return
        fecha_validada = validar_fecha(entry_fecha.get().strip())
        if fecha_validada is False:
            messagebox.showerror("Error",
                "Formato de fecha inválido.\nUsa YYYY-MM-DD, DD/MM/YYYY o DD-MM-YYYY.", parent=ventana)
            traer_al_frente(); return
        asignar_membresia(cliente, membresia, precio, pagado, fecha_validada)
        for e in entries: e.delete(0, ctk.END)
        recargar_tabla(); cargar_grafico_ingresos(); traer_al_frente()
        messagebox.showinfo("Exito", "Membresia asignada correctamente.", parent=ventana)

    ctk.CTkButton(frame_asignar, text="Asignar membresia", height=40,
                  command=asignar).grid(row=6, column=0, columnspan=2, pady=15)

    # ---------- ZONA DE PELIGRO ----------
    frame_emergencia = ctk.CTkFrame(scroll, corner_radius=18, fg_color="#1a0000")
    frame_emergencia.pack(fill="x", padx=20, pady=(10, 30))

    ctk.CTkLabel(frame_emergencia, text="Zona de peligro",
                 font=("Segoe UI", 16, "bold"), text_color="#FF4444").pack(anchor="w", padx=20, pady=(15, 2))
    ctk.CTkLabel(frame_emergencia,
                 text="Esta accion borra TODOS los clientes, suscripciones y pagos. No se puede deshacer.",
                 font=("Segoe UI", 12), text_color="gray60").pack(anchor="w", padx=20, pady=(0, 15))
    ctk.CTkLabel(frame_emergencia, text="Cambiar contraseña de administrador",
                 font=("Segoe UI", 13, "bold"), text_color="#FF8888").pack(anchor="w", padx=20, pady=(0, 8))

    frame_clave = ctk.CTkFrame(frame_emergencia, fg_color="transparent")
    frame_clave.pack(fill="x", padx=20, pady=(0, 20))

    entry_clave_actual    = ctk.CTkEntry(frame_clave, width=200, height=38,
                                         placeholder_text="Contraseña actual", show="*")
    entry_clave_actual.pack(side="left", padx=(0, 8))
    entry_clave_nueva     = ctk.CTkEntry(frame_clave, width=200, height=38,
                                         placeholder_text="Nueva contraseña", show="*")
    entry_clave_nueva.pack(side="left", padx=(0, 8))
    entry_clave_confirmar = ctk.CTkEntry(frame_clave, width=200, height=38,
                                         placeholder_text="Confirmar nueva contraseña", show="*")
    entry_clave_confirmar.pack(side="left", padx=(0, 8))

    def cambiar_clave():
        actual = entry_clave_actual.get().strip(); nueva = entry_clave_nueva.get().strip()
        confirmar = entry_clave_confirmar.get().strip(); clave_real = leer_clave()
        ...
        if nueva != confirmar:
            ...
            traer_al_frente(); return
        guardar_clave(nueva)
        entry_clave_actual.delete(0, ctk.END); entry_clave_nueva.delete(0, ctk.END)
    ...
    ctk.CTkButton(frame_clave, text="Cambiar", width=110, height=38,
                  fg_color="#8B4500", hover_color="#A05200",
                  command=cambiar_clave).pack(side="left")

    def borrar_todo():
        if not messagebox.askyesno("Borrar toda la base de datos",
            "Estas seguro? Se eliminaran clientes, suscripciones, pagos y alertas.\nEsta accion NO se puede deshacer.",
            parent=ventana):
            traer_al_frente(); return
        clave = simpledialog.askstring("Contrasena de administrador",
            "Ingresa la contrasena para continuar:", show="*", parent=ventana)
        if clave is None:
            traer_al_frente(); return
        if clave != leer_clave():
    ...
    ctk.CTkButton(frame_emergencia, text="BORRAR TODA LA BASE DE DATOS",
                  height=45, font=("Segoe UI", 14, "bold"),
                  fg_color="#8B0000", hover_color="#FF0000",
                  command=borrar_todo).pack(padx=20, pady=(0, 20))

    def _propagar(widget):
        if isinstance(widget, ttk.Treeview): return
        try: widget.bind("<MouseWheel>", _scroll_principal)
